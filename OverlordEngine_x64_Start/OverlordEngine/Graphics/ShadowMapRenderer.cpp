...
void ShadowMapRenderer::UpdateMeshFilter(const SceneContext& sceneContext, MeshFilter* pMeshFilter) const
{
	...
	//1. Figure out the correct ShadowGeneratorType (either Static, or Skinned) with information from the incoming MeshFilter
	ShadowGeneratorType shadowType
	{
		pMeshFilter->HasAnimations() ? ShadowGeneratorType::Skinned : ShadowGeneratorType::Static
	};
	//2. Retrieve the corresponding TechniqueContext from m_GeneratorTechniqueContexts array (Static/Skinned)
	const MaterialTechniqueContext& curTechnique{ m_GeneratorTechniqueContexts[int(shadowType)] };
	//3. Build a corresponding VertexBuffer with data from the relevant TechniqueContext you retrieved in Step2 (MeshFilter::BuildVertexBuffer)
...
void ShadowMapRenderer::Begin(const SceneContext& sceneContext)
{
	...
	//		*nearZ>0.1f
	//		*farZ>500.f
	const XMMATRIX projectionMatrix{ XMMatrixOrthographicLH(sceneContext.aspectRatio * 100.0f, 100.0f, 0.1f, 500.0f) };
	...
	const XMVECTOR lightPos{ XMLoadFloat4(&directionalLight.position) };
	const XMVECTOR lightDir{ XMLoadFloat4(&directionalLight.direction) };
	const XMMATRIX viewMatrix{ XMMatrixLookAtLH(lightPos, lightPos + lightDir, 
	...
	const XMFLOAT4X4& meshWorld, const std::vector<XMFLOAT4X4>& meshBones)
{
	...
	ShadowGeneratorType shadowType {pMeshFilter->HasAnimations() ? ShadowGeneratorType::Skinned : ShadowGeneratorType::Static};

	//2. Retrieve the correct TechniqueContext for m_GeneratorTechniqueContexts
	const MaterialTechniqueContext& techniqueContext{ m_GeneratorTechniqueContexts[int(shadowType)] };
	...
	for (const auto& m : pMeshFilter->GetMeshes())
	{
		//		- Set VertexBuffer
		constexpr UINT offset = 0;
		const auto& bufferData = pMeshFilter->GetVertexBufferData(techniqueContext.inputLayoutID, m.id);
		pDeviceContext->IASetVertexBuffers(0, 1, &bufferData.pVertexBuffer, &bufferData.VertexStride, &offset);
		//		- Set IndexBuffer
		pDeviceContext->IASetIndexBuffer(m.buffers.pIndexBuffer, DXGI_FORMAT_R32_UINT, 0);
		//		- Set correct TechniqueContext on ShadowMapMaterial - use ShadowGeneratorType as ID (BaseMaterial::SetTechnique)
		m_pShadowMapGenerator->SetTechnique(int(shadowType));
		...
		for (UINT p{}; p < techniqueDesc.Passes; ++p)
		{
			...
			pDeviceContext->DrawIndexed(m.indexCount, 0, 0);
		}
	}
}
