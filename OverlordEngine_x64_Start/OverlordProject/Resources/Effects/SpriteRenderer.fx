...
void CreateVertex(inout TriangleStream<GS_DATA> triStream, float3 pos, float4 col, float2 texCoord, float rotation, float2 rotCosSin, float2 offset, float2 pivotOffset)
{
    ...
    if (rotation != 0)
    {
        float2 newPosition = float2(originPos.x * rotCosSin.x - originPos.y * rotCosSin.y
            , originPos.y * rotCosSin.x + originPos.x * rotCosSin.y);

        geomData.Position = mul(float4(newPosition.x + offset.x, newPosition.y + offset.y, pos.z, 1.0f), gTransform);
    }
    else
    {
        geomData.Position = mul(float4(originPos.x + offset.x, originPos.y + offset.y, pos.z, 1.0f), gTransform);
    }
