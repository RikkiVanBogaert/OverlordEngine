...
void SpikeGenerator(triangle VS_DATA vertices[3], inout TriangleStream<GS_DATA> triStream)
{
    ...
    VS_DATA top = vertices[0];
    VS_DATA left = vertices[1];
    VS_DATA right = vertices[2];

	//Step 1. Calculate CENTER_POINT
    float3 basePoint = (top.Position + left.Position + right.Position) / 3;
	//Step 2. Calculate Face Normal (Original Triangle)
    float3 normal = normalize(top.Normal + left.Normal + right.Normal) / 3;
	//Step 3. Offset CENTER_POINT (use gSpikeLength)
    basePoint += gSpikeLength * normal;
    ...
    float3 v1 = (left.Position - top.Position);
    float3 v2 = (basePoint - top.Position);
    float3 spikeNormal = normalize(cross(v2, v1));

    CreateVertex(triStream, top.Position, spikeNormal);
    CreateVertex(triStream, left.Position, spikeNormal);
    CreateVertex(triStream, basePoint, spikeNormal);
    triStream.RestartStrip();

    v1 = (left.Position - right.Position);
    v2 = (basePoint - right.Position);
    spikeNormal = normalize(cross(v1, v2));

    CreateVertex(triStream, left.Position, spikeNormal);
    CreateVertex(triStream, right.Position, spikeNormal);
    CreateVertex(triStream, basePoint, spikeNormal);
    triStream.RestartStrip();

    v1 = (top.Position - right.Position);
    v2 = (basePoint - right.Position);
    spikeNormal = normalize(cross(v2, v1));
